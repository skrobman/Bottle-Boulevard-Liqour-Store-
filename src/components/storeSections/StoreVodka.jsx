...
import { vodkaData } from '../data/vodka'; // Updated import statement
...
const StoreVodka = () => {
    ...
    const brands = Array.from(new Set(vodkaData.vodkaData.map(element => element.brand)));

    const itemsPerPage = 12;
    const startIndex = (page - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;

    const filteredWhiskeys = vodkaData.vodkaData.filter(element => {
        ...
        <div className="main-section ml-14 mr-14 mt-3 flex justify-between">
            ...
            <div className="store-section bg-white">
                <div className="store-header p-7">
                    <h2 className='uppercase text-slate-700 font-semibold text-3xl'>vodka</h2>
                ...
                <ul className='store-section-elements'>
                    {whiskeysForPage.map(element => (
                        <li key={element.id} className='store-element'>
                            ...
                            <Link to={`/vodka/${element.id}`} className="store-name">{element.name}</Link>
